...
COHORTS = [
    ...
    # default 组 5/21-7/21 按"是否吃过暗保底(normal_kakuteiB)"整人二分。
    # 玩家归属用窗口内子查询判定; 两 folder 各含其玩家的全部旋转(所有数学表)。
    {
        "folder": "default_kakuteiB_players_5.21-7.21",
        "start": date(2026, 5, 21), "end": date(2026, 7, 21),
        "extra_where": ("t.partition_ab[0] = 'wytsuj-fothap-5Qixda' AND t.user_id IN ("
                        "SELECT DISTINCT s.user_id FROM public.fct_bet_orders s "
                        "WHERE s.game_id='SS03' AND s.status='COMPLETED' "
                        "AND s.currency_type IN ('CNY') "
                        "AND s.op_code NOT IN ('B26','TST','TSB','TSO') "
                        "AND s.partition_ab[0]='wytsuj-fothap-5Qixda' "
                        "AND s.math_table_id='normal_kakuteiB' "
                        "AND s.created_at >= '2026-05-21 00:00:00' "
                        "AND s.created_at < '2026-07-22 00:00:00')"),
    },
    {
        "folder": "default_clean_players_5.21-7.21",
        "start": date(2026, 5, 21), "end": date(2026, 7, 21),
        "extra_where": ("t.partition_ab[0] = 'wytsuj-fothap-5Qixda' AND t.user_id NOT IN ("
                        "SELECT DISTINCT s.user_id FROM public.fct_bet_orders s "
                        "WHERE s.game_id='SS03' AND s.status='COMPLETED' "
                        "AND s.currency_type IN ('CNY') "
                        "AND s.op_code NOT IN ('B26','TST','TSB','TSO') "
